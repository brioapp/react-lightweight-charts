...
class ChartWrapper extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      this.removeSeries()
      this.handleUpdateChart()
    } else if (prevProps.from !== this.props.from || prevProps.to !== this.props.to) this.handleTimeRange()
  }

  resizeHandler = () => {
    let width = this.props.autoWidth && this.chartDiv.current && this.chartDiv.current.parentNode.clientWidth
    let height =
  ...
  addSeries = (serie, type) => {
    ...
    let color = (serie.options && serie.options.color) || colors[this.series.length % colors.length]
    const series = this.chart[func]({
      color,
      ...serie.options,
    })
    let data = this.handleLinearInterpolation(serie.data, serie.linearInterpolation)
    series.setData(data)
  ...
  handleLinearInterpolation = (data, candleTime) => {
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      for (let interTime = prevTime; interTime < time - candleTime; interTime += candleTime) {
        ...
        let interValue = prevValue + (interTime - prevTime) * ((value - prevValue) / (time - prevTime))
  ...
  render() {
    let color = this.props.darkTheme ? darkTheme.layout.textColor : lightTheme.layout.textColor
